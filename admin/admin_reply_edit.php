<?php
    ...
    if (empty($replyrow)) {
        ...
        <script>
            alert('답글이 존재하지 않습니다..');
            self.location.href = './admin_board_view.php';
        </script>
    ");
    }
    // print_r($replyrow);
...
<body>
    ...
    <script src="./js/admin_reply_edit.js"></script>
    <div class="area"></div>
    <nav class="main-menu">
        <ul>
            <li>
        ...
        <div class="container text-center">
            ...
            <div class="pt-5 pb-5">
                <input type="text" name="subject" id="id_subject" class="form-control" placeholder="제목을 입력해 주세요."
                    autocomplete="off" value="<?= $replyrow['title'] ?>">
            </div>
            <div id="summernote"></div>
            <div class="mt-3 d-flex gap-2 justify-content-end">
                <button class="btn btn-primary" id="btn_write_submit">확인</button>
                <button class="btn btn-danger" id="btn_cancel">취소</button>
            </div>
        </div>
    </div>
    <script>
    ...
    var markupStr = `<?= $replyrow['content']; ?>`;
